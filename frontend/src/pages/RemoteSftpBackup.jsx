...
import { HardDriveDownload, FolderOpen, CheckCircle, XCircle, Loader2, ArrowRight } from 'lucide-react';
import clsx from 'clsx';

export default function RemoteSftpBackup() {
  ...
  useEffect(() => { api.get('/sftp-servers').then(({ data }) => setServers(data)); }, []);
  ...
  async function handleTest() {
    ...
    try {
      const { data } = await api.post(`/sftp-servers/${form.serverId}/test`);
      setTest(data);
  ...
  async function handleSubmit(e) {
    ...
    try {
      const { data } = await api.post('/backups/sftp-remote', { serverId: form.serverId, remotePath: form.remotePath });
      setResult(data);
  ...
  return (
    <div className="p-7 max-w-xl">
      <div className="page-header">
        <div>
          <h1 className="page-title">Sauvegarde SFTP</h1>
          <p className="page-sub">Récupérez un fichier ou dossier depuis un serveur SFTP</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card p-6 space-y-5">
        {/* Sélection serveur */}
        <div>
          <label className="label">Serveur SFTP</label>
          {servers.length === 0 ? (
            <p className="text-ink-300 text-sm">
              Aucun serveur configuré.{' '}
              <a href="/sftp-servers" className="text-brand hover:underline">Ajouter →</a>
            </p>
          ) : (
          ...
          <div className="flex items-center gap-3">
            ...
            {testResult && (
              <span className={clsx('flex items-center gap-1 text-sm', testResult.ok ? 'text-emerald-400' : 'text-red-400')}>
                {testResult.ok
                  ? <><CheckCircle size={13} /> Connexion OK</>
                  : <><XCircle size={13} /> {testResult.message}</>}
              </span>
            )}
          </div>
        )}
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="/home/testuser/upload/fichier.txt"
            />
          </div>
        ...
        {selectedServer && form.remotePath && (
          <div className="flex items-center gap-2 text-xs text-ink-200 bg-ink-600 border border-ink-500 rounded-lg p-3">
            <HardDriveDownload size={13} className="text-brand shrink-0" />
        ...
        <button
          type="submit"
          disabled={submitting || !form.serverId || !form.remotePath}
      ...
      {result && (
        <div className="mt-4 card p-5 space-y-3">
          <div className="flex items-center gap-2 text-emerald-400 font-semibold text-sm">
            <CheckCircle size={16} /> Sauvegarde SFTP réussie
          </div>
          ...
          <p className="text-xs text-emerald-400 font-medium pt-2 border-t border-ink-500">
            ✓ Fichier chiffré AES-256 et épinglé sur IPFS Cluster
          </p>
        </div>
      )}
    </div>
