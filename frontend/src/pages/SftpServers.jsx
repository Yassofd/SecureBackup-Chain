...
import { HardDriveDownload, Plus, Trash2, Pencil, CheckCircle, XCircle, Loader2 } from 'lucide-react';
...
function ServerForm({ initial, onSave, onCancel }) {
  ...
  return (
    <form onSubmit={submit} className="card p-5 space-y-4">
      <h3 className="text-sm font-semibold text-ink-50">{initial ? 'Modifier le serveur' : 'Ajouter un serveur SFTP'}</h3>
      {error && <p className="text-red-400 text-sm">{error}</p>}

      <div className="grid grid-cols-2 gap-3">
        <div className="col-span-2">
          <label className="label">Nom</label>
          <input required value={form.name} onChange={(e) => set('name', e.target.value)} className="input" placeholder="Mon serveur SFTP" />
        </div>
        <div>
        ...
        <div>
          ...
          <input required value={form.username} onChange={(e) => set('username', e.target.value)} className="input" placeholder="sftpuser" />
        </div>
        <div>
        ...
        <div className="col-span-2">
          ...
          <input value={form.description} onChange={(e) => set('description', e.target.value)} className="input" placeholder="Serveur SFTP de production" />
        </div>
      </div>
...
export default function SftpServers() {
  ...
  async function load() {
    const { data } = await api.get('/sftp-servers');
    setServers(data); setLoading(false);
  }

  useEffect(() => { load(); }, []);

  async function handleAdd(payload)        { await api.post('/sftp-servers', payload); setShowForm(false); load(); }
  async function handleUpdate(id, payload) { await api.put(`/sftp-servers/${id}`, payload); setEditing(null); load(); }
  async function handleDelete(id)          { if (!confirm('Supprimer ce serveur SFTP ?')) return; await api.delete(`/sftp-servers/${id}`); load(); }

  async function handleTest(id) {
    setTesting(id); setTestResults((r) => ({ ...r, [id]: null }));
    try {
      const { data } = await api.post(`/sftp-servers/${id}/test`);
  ...
  return (
    <div className="p-7">
      <div className="page-header">
        <div>
          <h1 className="page-title">Serveurs SFTP</h1>
          <p className="page-sub">Connexions SFTP pour sauvegardes et restaurations distantes (sans exécution de commandes shell)</p>
        </div>
        {canEdit && !showForm && (
      ...
      {loading ? (
        ...
        <div className="text-center py-16 card">
          <HardDriveDownload size={36} className="mx-auto mb-3 text-ink-400" />
          <p className="text-ink-300 text-sm">Aucun serveur SFTP configuré</p>
          <p className="text-ink-400 text-xs mt-1">Idéal pour les serveurs dédiés SFTP (AWS Transfer Family, WinSCP…)</p>
        </div>
      ) : (
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="p-2 bg-ink-600 border border-ink-500 rounded-lg">
                      <HardDriveDownload size={16} className="text-brand" />
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {canEdit && (
                      <>
                        <button onClick={() => handleTest(s.id)} disabled={testing === s.id} title="Tester la connexion" className="p-2 text-ink-300 hover:text-brand disabled:opacity-40 transition-colors rounded-lg hover:bg-ink-600">
